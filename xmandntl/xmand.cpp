...
int main(int argc, char*argv[])
{
    ...
    RR obs_x_width, obs_y_height;
    RR sub_pixel_real, sub_pixel_imag;
    RR pixel_real_width, pixel_imag_height; 
    RR magnify, real_translate, imag_translate;
    RR x_prime, y_prime;
    ...
    if ( ( argc < 5 ) && ( argc > 1 ) ) {
        ...
        fprintf(stderr,"     :          precision_integer (optional)\n");
        ...
        if ( ( candidate_int < 4 ) || ( candidate_int > 65536 ) ){
        ...
        magnify = conv<RR>(argv[2]);
        real_translate = conv<RR>(argv[3]);
        imag_translate = conv<RR>(argv[4]);

        if (argc >= 6) {
            candidate_int = (int)strtol(argv[5], (char **)NULL, 10);
            if ( ( errno == ERANGE ) || ( errno == EINVAL ) ){
                fprintf(stderr,"FAIL : precision_integer not understood\n");
                perror("     ");
                return ( EXIT_FAILURE );
            }
            if ( ( candidate_int < 53 ) || ( candidate_int > 1000000 ) ){
                fprintf(stderr,"WARN : precision is unreasonable\n");
                fprintf(stderr,"     : we shall assume 53 and proceed.\n");
                RR::SetPrecision(53);
            } else {
                RR::SetPrecision((uint32_t)candidate_int);
            }
        } else {
            RR::SetPrecision(53);
        }
    ...
    printf("translate = ( %s , %s )\n",
                                      rr_to_str(real_translate).c_str(), rr_to_str(imag_translate).c_str() );

    printf("  magnify = %s\n", rr_to_str(magnify).c_str() );
    printf("precision = %i\n\n", RR::precision() );
    ...
    while(1){
        ...
        if ( button == Button1 ){
            ...
            if (    ( mouse_x >=  offset_x ) && ( mouse_y >= offset_y )
                ...
                fprintf(stderr,"c = ( %s , %s )\n", rr_to_str(x_prime).c_str(), rr_to_str(y_prime).c_str() );

                XSetForeground(dsp, gc3, red.pixel);
                sprintf(buf,"   select = ( %s , %s )", rr_to_str(x_prime).c_str(), rr_to_str(y_prime).c_str() );
                ...
                sprintf(buf,"  magnify = %s", rr_to_str(magnify).c_str());
                XDrawImageString( dsp, win3, gc3, 10, 120, buf, (int)strlen(buf));
                sprintf(buf,"   centre = ( %s , %s )", rr_to_str(real_translate).c_str(), rr_to_str(imag_translate).c_str());
                XDrawImageString( dsp, win3, gc3, 10, 140, buf, (int)strlen(buf));
                sprintf(buf,"precision = %i bits", RR::precision());
                XDrawImageString( dsp, win3, gc3, 10, 160, buf, (int)strlen(buf));
                ...
                sprintf(buf,"pixel_real_w = %s", rr_to_str(pixel_real_width).c_str());
                XDrawImageString( dsp, win3, gc3, 10, 200, buf, (int)strlen(buf));
                pixel_imag_height = obs_y_height / (double)eff_height;
                sprintf(buf,"pixel_imag_h = %s", rr_to_str(pixel_imag_height).c_str());
                XDrawImageString( dsp, win3, gc3, 10, 220, buf, (int)strlen(buf));
